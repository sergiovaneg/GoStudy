...
func (t *Traversed) merge(tOther Traversed) {
	for k := range tOther {
		(*t)[k] = true
	}
}
...
func (pr *ParallelRecord) parallelBFS(w Walker) {
...
func (pr *ParallelRecord) printFinalState() {
	pr.mu.Lock()
	m := make(Maze, len(pr.m))
	for i, row := range pr.m {
